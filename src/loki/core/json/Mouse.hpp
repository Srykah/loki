...
struct adl_serializer<sf::Mouse::Button> {
 private:
  static constexpr std::pair<sf::Mouse::Button, const char*> MOUSE_BUTTON_NAMES[] = {
#define LOKI_JSON_MOUSE_BUTTON_NAME(Button) {sf::Mouse::Button, #Button}
      LOKI_JSON_MOUSE_BUTTON_NAME(Left), LOKI_JSON_MOUSE_BUTTON_NAME(Right), LOKI_JSON_MOUSE_BUTTON_NAME(Middle),
      LOKI_JSON_MOUSE_BUTTON_NAME(XButton1), LOKI_JSON_MOUSE_BUTTON_NAME(XButton2)
#undef LOKI_JSON_MOUSE_BUTTON_NAME
  };

 public:
  static void to_json(json& j, sf::Mouse::Button button) {
    auto it = std::find_if(std::begin(MOUSE_BUTTON_NAMES), std::end(MOUSE_BUTTON_NAMES),
