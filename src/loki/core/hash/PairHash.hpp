...
/// \note This functor only makes sense if the arguments of the pair can compare
/// equal to one another... \tparam Pair the type of the std::pair
template <class Pair>
struct PairHashSym {
...
/// \ingroup UTILS_COMBINEHASH
/// \brief Helper functor for comparing pairs where order of arguments doesn't
/// matter \warning This functor only exists for pairs where the two arguments
/// can compare equal to one another ! \tparam Pair1 the type of the first
/// std::pair \tparam Pair2 the type of the second std::pair
