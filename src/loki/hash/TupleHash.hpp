...
struct TupleHash {
    ...
    size_t operator()(const Tuple& val) const {
        ...
        // std::apply allows us to use combineHash with all values stored in the tuple at once
    }
};
...
/// \note This functor only makes sense if all arguments of the tuple can compare equal to one another...
/// \tparam Tuple the type of the std::tuple
template <class Tuple>
struct TupleHashSym {
    ...
    size_t operator()(const Tuple& val) const {
        ...
        // std::apply allows us to use combineHashSym with all values stored in the tuple at once
    }
};
