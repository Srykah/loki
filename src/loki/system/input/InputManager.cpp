...
void InputManager::setPlayerConfig(PlayerConfig&& playerConfig,
                                   PlayerId playerId) {
  ...
  for (const auto& [playerId, playerConfig] : playerConfigs) {
    ...
    for (const auto& [inputName, inputTriggers] : inputConfig) {
      ...
      for (const auto& trigger : inputTriggers) {
        std::tie(active, value) =
            getTriggerStatus(trigger, playerConfig.inputMethod);
        if (active) {
          break;
        }
      }
...
InputState InputManager::getInputState(const InputId& inputId,
                                       PlayerId playerId) const {
  return inputStates.at(playerId).at(inputId);
}

std::pair<bool, float> InputManager::getTriggerStatus(
  ...
  if (std::holds_alternative<MouseButtonTrigger>(trigger) &&
    ...
    return {sf::Joystick::isButtonPressed(getJoystickId(inputMethod),
                                          joystickButton),
            1.f};
  } else if (std::holds_alternative<JoystickAxisTrigger>(trigger) &&
             inputMethod & InputMethod::Joystick) {
    const auto& axisTrigger = std::get<JoystickAxisTrigger>(trigger);
    auto axisPos = sf::Joystick::getAxisPosition(getJoystickId(inputMethod),
                                                 axisTrigger.axis);
    bool activeUp =
        axisTrigger.direction != JoystickAxisTrigger::Direction::NEGATIVE &&
        axisPos > deadZones.at(axisTrigger.axis);
    bool activeDown =
        axisTrigger.direction != JoystickAxisTrigger::Direction::POSITIVE &&
        axisPos < -deadZones.at(axisTrigger.axis);
