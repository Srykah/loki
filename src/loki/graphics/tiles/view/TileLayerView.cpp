...
void TileLayerView::update(const sf::Time& delta) {
  for (auto&& [tileId, tileViewDatum]: tileViewData) {
    ...
    while (tileViewDatum.timeSinceLastFrame >= tileData.animation.at(tileViewDatum.frameIndex).duration) {
      tileViewDatum.timeSinceLastFrame -= tileData.animation.at(tileViewDatum.frameIndex).duration;
      tileViewDatum.frameIndex = (tileViewDatum.frameIndex + 1) % tileData.animation.size();
      needUpdate = true;
    }
    if (needUpdate) {
      for (int x{0}; x < gridSize.x; ++x) {
        for (int y{0}; y < gridSize.y; ++y) {
          if (layerData.data[{x,y}]-1 == tileId) {
            setTile(x,y,tileData.animation.at(tileViewDatum.frameIndex).tileId);
          }
        }
      }
    }
...
void TileLayerView::initVertices(unsigned int x, unsigned int y) {
  ...
  auto tilePositionOrigin = sf::Vector2f (
      tileWidth * x,
      tileHeight * y
  );
