...
template <>
struct adl_serializer<loki::graphics::PropertyMap> {
  static void from_json(const json& j, loki::graphics::PropertyMap& pm) {
    for (const auto& datum : j) {
      auto name = datum.at("name").get<std::string>();
      auto type = datum.at("type").get<std::string>();
      loki::graphics::PropertyData value;
      if (type == "string") {
  ...
  static void to_json(json& j, const loki::graphics::PropertyMap& pm) {
